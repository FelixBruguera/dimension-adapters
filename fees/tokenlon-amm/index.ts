...
const contracts = {
  ...
  rfqV2: "0x91c986709bb4fe0763edf8e2690ee9d5019bea4a",
...
const abis = {
  ...
  SwappedV2:
    "event Swapped((string source, bytes32 transactionHash, uint256 settleAmount, uint256 receivedAmount, uint16 feeFactor, uint16 subsidyFactor) quote, (address makerAddr, address takerAssetAddr, address makerAssetAddr, uint256 takerAssetAmount, uint256 makerAssetAmount, address userAddr, address receiverAddr, uint256 salt, uint256 deadline) order)",
};

const toBigInt = (value: any) => BigInt(value?.toString() ?? 0);
...
const fetch = async ({ createBalances, getLogs }: FetchOptions): Promise<FetchResultV2> => {
  const dailyFees = createBalances();

  const [pmmLogs, rfqLogs, rfqV2Logs, ammWrapperLogs, ammWrapperV2Logs] = await Promise.all([
    getLogs({
      target: contracts.pmm,
      eventAbi: abis.FillOrder,
    }),
    getLogs({
      target: contracts.rfq,
      eventAbi: abis.FillOrder,
    }),
    getLogs({
      target: contracts.rfqV2,
      eventAbi: abis.FilledRFQ,
    }),
    getLogs({
      target: contracts.ammWrapperWithPath,
  ...
  [...pmmLogs, ...rfqLogs].forEach((log: any) => {
  ...
  ammWrapperLogs.forEach((log: any) => {
    const fee = getAmmWrapperFee(log, log.makerAssetAmount);
    if (fee > ZERO) dailyFees.add(log.makerAssetAddr, fee, METRIC.SWAP_FEES);
  });

  ammWrapperV2Logs.forEach((log: any) => {
